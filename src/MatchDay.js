...
export function MatchDay({ matches, today, onMatchClick, onTeamSelect, onTT, onMoveTT, onHideTT }) {
  ...
  // Show today + all upcoming, grouped by date
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={styles.panelHdr}>
        <span style={{ color: 'var(--ac-gold)', fontWeight: 500 }}>⚡ Match Schedule</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {dates.length === 0 ? (
          <div style={styles.noMatch}>
            ...
            <div>Tournament has concluded</div>
          </div>
        ) : dates.map(date => {
...
const styles = {
  ...
  dateHdr: { padding: '5px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 10, borderBottom: '1px solid var(--bd-main)', position: 'sticky', top: 0, zIndex: 1 },
