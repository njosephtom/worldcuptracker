...
const BD = {
  ...
  74: { r:'R32', p:2,  h:'1I',  a:'2I',        pid:89          },
  75: { r:'R32', p:3,  h:'2E',  a:'2G',        pid:89          },
  ...
  83: { r:'R32', p:11, h:'2D',  a:'3+BEFIJ',   pid:93          },
  84: { r:'R32', p:12, h:'1D',  a:'3+AEHIJ',   pid:94          },
  85: { r:'R32', p:13, h:'1G',  a:'3+CEFIJ',   pid:94          },
  86: { r:'R32', p:14, h:'1B',  a:'3+DEIJL',   pid:95          },
  87: { r:'R32', p:15, h:'1K',  a:'2I',        pid:95          },
...
// ─── Merge mock results into bracket entries ──────────────────────────────────
const BDM = {};   // BD with Mock – real names + scores where available
Object.entries(BD).forEach(([id, m]) => {
  const r = MOCK_RESULTS[+id];
  BDM[+id] = r ? { ...m, h:r.h, a:r.a, hs:r.hs, as:r.as } : m;
});

// ─── Path highlight (traces a team through BDM from R32 to Final) ─────────────
...
function ConnectorLines({ highlighted }) {
  const paths = useMemo(() => {
    ...
    for (const [id, m] of Object.entries(BD)) {
      ...
      const px = cardX(m.r), py = cardCY(m.r, m.p);
      for (const kid of m.kids) {
        const km = BD[kid]; if (!km) continue;
        const kx = cardX(km.r), ky = cardCY(km.r, km.p);
        const mx = kx + CW + GX / 2;
...
// ─── Team slot (with optional score badge + winner gold) ─────────────────────
...
function MatchCard({ id, m, highlighted, hoveredSlot, onSlotEnter, onSlotLeave, onSlotClick }) {
  ...
  return (
    <div style={{
      ...
      border:`1px solid ${(isOnPath || (isFinal && fin)) ? 'rgba(240,192,64,0.6)' : 'var(--brk-bd)'}`,
      borderRadius:5, overflow:'hidden',
      boxShadow: isFinal && fin
        ? '0 0 28px rgba(240,192,64,0.45)'
        : isOnPath ? '0 0 10px rgba(240,192,64,0.15)' : 'none',
...
function BronzeCard() {
  ...
  const x = cardX('Final'), y = TH - SH * 2.5;
  const fin      = !!mock;
...
// ─── Champion banner (shown below the Final card) ─────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:8, marginBottom:4 }}>
        ...
        <span style={{ fontSize:'var(--fs-base)', fontWeight:800, color:'var(--ac-gold)', letterSpacing:0.3 }}>
          {champion}
        </span>
      </div>
...
function BracketGroupCard({ g, standings, onTT, onMoveTT, onHideTT }) {
  ...
  return (
    <div style={{ border:'1px solid var(--brk-bd)', borderRadius:5, marginBottom:7, flexShrink:0, width:CW }}>
      <div style={{ background:'var(--brk-div)', padding:'4px 7px', fontSize:9, fontWeight:700,
        color:'var(--ac-gold)', textTransform:'uppercase', letterSpacing:0.8, borderRadius:'4px 4px 0 0' }}>
        Group {g}
      </div>
      {teams.map((t, i) => {
        ...
        return (
          <div key={t.name}>
            <div onClick={() => setActiveTeam(isActive ? null : t.name)}
              onMouseEnter={e => onTT?.(e, ttHtml)} onMouseMove={onMoveTT} onMouseLeave={onHideTT}
...
function DesktopBracket({ standings, hoveredSlot, setHoveredSlot, onTT, onMoveTT, onHideTT }) {
  ...
  // Trophy is positioned below ChampionBanner — push it down further when mock is active
...
// ─── Mobile: knockout match row (with scores) ─────────────────────────────────
function MobileMatchRow({ id, m, highlighted, hoveredSlot, onSlotToggle }) {
  ...
  return (
    <div style={{
      background: isOnPath ? 'rgba(240,192,64,0.06)' : 'var(--brk-cell)',
      border:`1px solid ${(isOnPath || (isFinal && fin)) ? 'rgba(240,192,64,0.5)' : 'var(--brk-bd)'}`,
      ...
      <TeamRow slot={m.h} score={fin ? m.hs : undefined} wins={homeWins} />
      ...
      <TeamRow slot={m.a} score={fin ? m.as : undefined} wins={awayWins} />

      {/* Champion label for Final */}
      {isFinal && fin && (
        <div style={{ background:'rgba(240,192,64,0.1)', borderTop:'1px solid rgba(240,192,64,0.3)',
...
function MobileBracket({ standings, hoveredSlot, setHoveredSlot, onTT, onMoveTT, onHideTT }) {
  ...
  const highlighted = useMemo(() => pathFor(hoveredSlot), [hoveredSlot]);
  const toggleSlot  = (slot) => setHoveredSlot(s => s === slot ? null : slot);
        ...
        {MOBILE_ROUNDS.map(({ key, label }) => {
          ...
          return (
            <button key={key} onClick={() => { setActiveRound(key); setHoveredSlot(null); }} style={{
              flexShrink:0, padding:'9px 14px', fontSize:'var(--fs-sm)', fontWeight:on?700:500,
              color:on?'var(--ac-gold)':'var(--brk-dim)', background:'none', border:'none',
