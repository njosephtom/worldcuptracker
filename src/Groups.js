      ...
      {Object.keys(GROUPS).map(g => {
        ...
        return (
          <div key={g} style={{ background: 'var(--bg-card)', border: '1px solid var(--bd-card)', borderRadius: 8, overflow: 'hidden' }}>
            <div style={{ background: 'var(--bg-panel)', padding: '5px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--ac-gold)', textTransform: 'uppercase', letterSpacing: '.8px' }}>Group {g}</span>
            ...
            {sorted.map((t, i) => (
              <div key={t.name} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderBottom: '1px solid var(--bg-inner)', fontSize: 10, background: i < 2 ? 'var(--bg-qualified)' : (i % 2 === 0 ? 'var(--bg-stripe)' : 'transparent'), borderLeft: i < 2 ? '2px solid var(--ac-green)' : '2px solid transparent', cursor: 'pointer' }} onClick={() => onTeamSelect?.(t.name)}>
                ...
                {[t.mp,t.w,t.d,t.l].map((v,j) => <span key={j} style={{ fontSize: 9, color: 'var(--tx-muted)', width: 18, textAlign: 'center' }}>{v}</span>)}
                <span style={{ fontSize: 9, color: 'var(--tx-muted)', width: 20, textAlign: 'center' }}>{t.gf-t.ga}</span>
                <span style={{ fontSize: 9, fontWeight: 600, color: 'var(--tx-primary)', background: 'var(--bg-inner)', padding: '1px 4px', borderRadius: 2, minWidth: 18, textAlign: 'center' }}>{t.pts}</span>
              </div>
            ))}
            {played.length > 0 && (
              <div style={{ padding: '5px 10px 4px', borderTop: '1px solid var(--bg-inner)' }}>
                <div style={{ fontSize: 8, color: 'var(--tx-dim2)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 3 }}>Results</div>
                {played.map(m => (
                  <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, padding: '2px 0', color: 'var(--tx-muted)' }}>
                    <span>{flagLabel(m.h)} <b style={{color:'var(--ac-gold)'}}>{m.homeScore}–{m.awayScore}</b> {flagLabel(m.a)}</span>
                    <span style={{ color: 'var(--tx-dim2)' }}>{m.d.slice(5)}</span>
                  </div>
                ))}
              </div>
            )}
            {upcoming.length > 0 && (
              <div style={{ padding: '5px 10px 6px', borderTop: '1px solid var(--bg-inner)' }}>
                <div style={{ fontSize: 8, color: 'var(--tx-dim2)', textTransform: 'uppercase', letterSpacing: '.5px', marginBottom: 3 }}>Upcoming</div>
                {upcoming.map(m => (
                  <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, padding: '2px 0', color: 'var(--tx-dim)' }}>
